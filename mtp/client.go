...
type DialOption func(f *Client) error

// Dial Connects to the fcgi responder at the specified network address.
// See func net.Dial for a description of the network and address parameters.
func Dial(network, address string, options ...DialOption) (*Client, error) {
	f := &Client{
		keepAlive:      false,
		reqId:          1,
	...
	for _, o := range options {
		if err := o(f); err != nil {
			return nil, errors.Wrap(err, "DialOption failed")
		}
	}
...
func (this *Client) writeRecord(recType uint8, content []byte) (err error) {
	this.mutex.Lock()
...
func (this *Client) writeBeginRequest(role uint16, flags uint8) error {
	b := [8]byte{byte(role >> 8), byte(role), flags}
	return this.writeRecord(FCGI_BEGIN_REQUEST, b[:])
}

func (this *Client) writeEndRequest(appStatus int, protocolStatus uint8) error {
	b := make([]byte, 8)
...
func (this *Client) writePairs(recType uint8, pairs map[string]string) error {
...
func (this *Client) Do(p map[string]string, req io.Reader) (r io.Reader, err error) {
...
// from fcgi responder
func (this *Client) Request(header http.Header, body io.Reader) (respHeader http.Header, respBody []byte, err error) {
	p := map[string]string{}
	for k, v := range header { //TODO header length
		p[k] = v[0]
	}

	r, err := this.Do(p, body)
	if err != nil {
		return
	}
